...
% \usepackage[UTF8]{ctex}     % Chinese support (Commented out for the English version)
...
% Code block style settings (Removed language specific dependencies to prevent compile errors)
\lstset{
...
\title{\textbf{Photonic Quantum Measurement and Control System Communication Protocol Document}}
\author{OriginQ}
\date{\today}
...
\begin{itemize}
    ...
    \item \textbf{ConvertQProg} (\texttt{String}): [Required] Quantum program; the instructions to be executed by the measurement and control system (Photonic platform specific format).
...
The basic gate types for the Photonic platform differ from those of the superconducting platform. The \texttt{ConvertQProg} field maintains a dedicated format for the Photonic platform and is not forced to be consistent with the superconducting platform.
